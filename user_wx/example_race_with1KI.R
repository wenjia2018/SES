...
example0 %>% saveRDS("./user_wx/example_race_with1KI_withses4AncestryPC.rds") 


# example1 =
#   args %>%
#   filter(treatment %in% c("raceethnicity", "raceethnicity2", "raceethnicity3"),
#   gene_set_name == "whole_genome_and_tfbm",
#   names(controls) == "all") %>%
#   mutate(out = pmap(., safely(model_fit), funcs),
#          controls = names(controls))
# example1 %>% saveRDS("./user_wx/example_race_detfbm_withses4AncestryPC.rds")
