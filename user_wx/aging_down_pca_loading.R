#' ---
#' title: Aging down PCA loading per cluster plotting
...
#' #### Figure Aging down PCA loadings
...
fig1A = function(dim){
  ...
  a <-
    ex0_m7pca %>%
    filter(gene_set_name == "Aging Down" & treatment == "Light Medium") %>%
    filter(p_id == dim) %>%
    pull(loadings) %>%
    as.data.frame() %>%
  ...
  a %>% 
    ...
    labs(
      ...
      title = paste("Figure . PCA", dim, "loading from aging down for aging subsets and clusters"),
