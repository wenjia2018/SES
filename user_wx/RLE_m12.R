...
#+ echo=F, eval=T, warning=FALSE, message=FALSE,comment=NA
if(0){
  data_DE$ttT %>% map(~ filter(.x, adj.P.Val<0.05)) %>%  set_names(data_DE$treatment) %>%  openxlsx::write.xlsx("./user_wx/RLE_DE_ses.xlsx")
...
for (i in 1:5) {
  ...
  name = names(temp)[i]
  write(temp[[i]], file = str_c(name, "_down.txt"))
}
  for (i in 1:5) {
    temp = 
      data_DE$ttT %>% 
    ...
    name = names(temp)[i]
    write(temp[[i]], file = str_c(name, "_up.txt"))
  }
}
...
#+ echo=F, eval=T, warning=FALSE, message=FALSE,comment=NA
example0_with1k <- readRDS("~/ses-1/user_wx/example_RLE_DE_m12_nonparalle.rds")

data_logfc_with1k = example0_with1k %>%  hoist(out, m = list("result",  "m12_fdr", 1, "other", "m"))

data_logfc_with1k %>% DE_logFC_ploting(caption_text = "withinflamation 1K genes")



example0_without1k  <- readRDS("~/ses-1/user_wx/example_RLE_DE_m12_removeinflame.rds")
