...
X <- dat %>% phenoData() %>% .@data %>% dplyr::select(all_of(control), all_of(treatment)) %>%
  mutate(ses_squared = ses_sss_composite * ses_sss_composite) 
...
extract_lm = function(x) x %>% tidy %>% filter(str_detect(term, "ses4modelses_sss_composite|ses4modelses_squared"))

a = out %>% map(extract_lm)
sink("./user_wx/lm_21gene_nonlinear.txt")
a %>% print 
sink()
