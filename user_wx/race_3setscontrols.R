...
#' ---
#' title: race pattern with basic control, ses composite control and ancestryPC control
...
library(here)
library(tidyverse)
example_race_with1KI <- readRDS("~/ses-1/user_wx/example_race_dummy_09.11.rds")
race_ge_tfbm <- readRDS("~/ses-1/user_wx/race_detfbm_dummy_09.11.rds")
...
#+ echo=F, eval=T, warning=FALSE, message=FALSE
 ...
 med_present(focal = "totdiscrim1_category", data = example_race_with1KI)  %>%
   kableExtra::kable() %>%
   kableExtra::kable_styling()
 #' ### "totdiscrim1_gamma" mediation
 #+ echo=F, eval=T, warning=FALSE, message=FALSE
 
 # med_present(focal = "totdiscrim1_gamma", data = example_race_with1KI)  %>%
 #   kableExtra::kable() %>%
 #   kableExtra::kable_styling()
 #' ### "totdiscrim2_gamma"  mediation
 #+ echo=F, eval=T, warning=FALSE, message=FALSE
 
 # med_present(focal = "totdiscrim2_gamma" , data = example_race_with1KI)  %>%
 #   kableExtra::kable() %>%
 #   kableExtra::kable_styling()
 #' ## "countdiscrimwhy_gamma" mediation
 #+ echo=F, eval=T, warning=FALSE, message=FALSE
 
 # med_present(focal = "countdiscrimwhy_gamma", data = example_race_with1KI)  %>%
 #   kableExtra::kable() %>%
 #   kableExtra::kable_styling()
 # example_race_with1KI %>%
 # hoist(out, "result") %>%
 #   hoist(result, "m7_ob") %>% 
 #   unnest(matches("^m7")) %>% 
 #   hoist(m7_ob, result = list("mediation", "countdiscrimwhy_gamma" , "result")) %>% 
 #   filter(result!="NULL") %>% 
 #   unnest_longer(result) %>% 
 #   hoist(result,p = "p")%>% 
 #   select(1:4,6)  %>%
