#' ---
#' title: Aging composite PCA loading per cluster plotting
...
a <-
  ex0_m7pca %>%
  filter(gene_set_name == "Aging Composite" & treatment == "Light Medium") %>%
  filter(p_id == dim) %>%
  pull(loadings) %>%
  as.data.frame() %>%
...
labs(
  ...
  title = paste("Figure . PCA", dim, "loading from aging composite for aging subsets and clusters"),
