...
darkblack = temp %>%
  filter(treatment %>% str_detect("DarkBlack")) %>%
  filter(!(gene_set_name %in% c("Lysosome Metabolism (n=5)","Fatty Acid Metabolism (n=7)","Immune Genes (n=44)","DNA Replication (n=16)")))

 data1 = darkblack %>% pull(well_loaded) %>% set_names(darkblack$gene_set_name)
 data1 %>% map(~ crossprod(table(stack(.))))
...
#+ echo=F, eval=T, warning=FALSE, message=FALSE, fig.width=12, fig.height=10
 lightmed = temp %>%
   ...
   filter((gene_set_name %in% c("Aging Composite (n=1048)", "Aging Cluster Complement (n=818)", "Aging Up (n=438)","Actin Regulation (n=9)")))

 data2 = lightmed %>% pull(well_loaded) %>% set_names(lightmed$gene_set_name)
 data2 %>% map(~ crossprod(table(stack(.))))
