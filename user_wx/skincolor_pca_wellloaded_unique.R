...
#' ---
#' title: unique well loaded genes at varying threshold 
...
unique_all = function(data) {
  out = list()
  n = length(data)
  for (i in 1: n) {
    rest = data[-i]
    rest_union = reduce(rest, union)
    out[[i]] = data[[i]] %>% setdiff(rest_union) 
  } 
  out = out %>% setNames(names(data))
  return(out)
}

unique_genes = function(data, threshold, gene_list = FALSE) {
  temp2 = 
    data %>%
  ...
  if(gene_list == FALSE){
    temp3 %>% 
      ...
      map_depth(2,length) %>% 
      unlist(recursive = F) %>% 
      enframe(name ="gene_set_name") %>% 
      rename(!!quo_name(str_c("threshold = ",threshold)) := value)
  } else{
    out = 
      temp3 %>% 
      map(unique_all)
...
#' ### numbers of unique well loaded genes from threshold 0.1 to 0.5
#+ echo=F, eval=T, warning=FALSE, message=FALSE, fig.width=12, fig.height=10
aa = seq(0.1, 0.5, 0.1) %>% map(~ unique_genes(outm71, .x)) %>% reduce(left_join)
aa %>% kableExtra::kable() %>% kableExtra::kable_styling()


save_list_to_excel = function(data, threshold){
  bb = unique_genes(outm71, threshold, gene_list = TRUE)
  library(stringi)
  bb = 
    bb %>% 
    map( ~ stri_list2matrix(.x, fill=''))
  bb %>% openxlsx::write.xlsx(str_c("./user_wx/wellloaded_",threshold,".xlsx"))
}
aa = seq(0.1, 0.5, 0.1) %>% map(~ save_list_to_excel(outm71, .x)) 
