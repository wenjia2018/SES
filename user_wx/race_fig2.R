...
#' #### skin color within nonhispanic black strata
#' (contrast is between darkblack and light medium, white only have 1 subject,not estimable)
...
example = readRDS("/home/xu/ses-1/user_wx/color3_bespoke_NonHblack_strata_16.03.2021.rds")
...
fig1A = function(p_eqtl){
  ...
  ggplot(ex0_m8fdr, aes(treatment, gene_set_name, size = pval2
    ...
    labs(
      ...
      title = "Figure 2. Associations between Skin Color
            and mRNA-Based Disease Signatures, Add Health
            (p-values reported, FDR-corrected for whole genome)",
      y = "mRNA Signatures",
      x = "Skincolor") +
    scale_x_discrete(breaks=c("Light Medium"),
                     labels=c("Light Medium v.s Dark Black"))+
    theme(
