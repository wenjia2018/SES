...
#+ echo=F, eval=T, warning=FALSE, message=FALSE 
a = map2(white %>% select(3:6), black %>% select(3:6), wilcox.test) %>% map(~ .$p.value) %>% as.data.frame() %>% `rownames<-`("white_black")
b = map2(white %>% select(3:6), hispanic %>% select(3:6), wilcox.test) %>% map(~ .$p.value) %>% as.data.frame() %>%  `rownames<-`("white_hispanic")
...
#+ echo=F, eval=T, warning=FALSE, message=FALSE 
a = map2(white_blood %>% select(3:6), black_blood %>% select(3:6), wilcox.test) %>% map(~ .$p.value) %>% as.data.frame() %>% `rownames<-`("white_black")

b = map2(white_blood %>% select(3:6),hispanic_blood %>% select(3:6), wilcox.test) %>% map(~ .$p.value) %>% as.data.frame() %>%  `rownames<-`("white_hispanic")
