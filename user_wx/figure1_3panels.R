...
all_pathway = m7_ob %>%
  ...
  hoist(enrichment_of_well_loaded_genes_on_significant_PCs, pathway = list("out","enriched_physiology","kegg")) 
...
panelT1 = tableGrob(temp[1:4, ] %>% dplyr::select(2,3), rows = NULL)
panelT2 = tableGrob(temp[5:8, ] %>% dplyr::select(2,3), rows = NULL)
panelT3 = tableGrob(temp[9:10, ] %>% dplyr::select(2,3), rows = NULL)
