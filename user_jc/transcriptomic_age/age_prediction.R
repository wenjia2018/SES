...
controls = controls %>%
  pluck("basic") %>% 
  setdiff(c("age_w5", "BirthY")) %>% # because now interested in these as treatments, not controls
  setdiff(c("W5REGION", "re")) %>% 
  append("smoking_biow5")

# apparently Brandt calculated a good age score...
age = left_join(enframe(as.integer(dat$AID)), 
                data.table::fread("/home/share/projects/ACEs/transcriptomic.age.predictions.1433genes.allcovs.general.070720")[,1:2 ], 
                by = c("value" = "aid"))
dat$age = age$age
