model_MR = function(gene_set_name, treatment, IV, control){
  ...
  data = pData(dat) %>% dplyr::select(all_of(control), all_of(treatment), IV) %>% 
 ...
 # for genetic(PGSBMI) associations with the outcomes(PCs) 
  datt =
    prepro(gene_set, IV, control) %>% 
    dplyr::rename(IV = IV) %>% 
    remove_diseased_subjects_from_datt(gene_set_name, control)
