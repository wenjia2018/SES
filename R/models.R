abbreviations =
  tribble(
    ...
    "m13", "DE by design matrix by TMM within each predefined signature with bootstrap option",
    "m10", "DE, TFBM and gene set analysis within each predefined signature",
    "m11", "TFBM enrichment for predefined signatures",
    "m16", "Sprse PCA and regression",
...
fit_pca_util = function(datt, gene_set, rotate, ncomp){
  ...
  if (length(gene_set) < ncomp) ncomp = length(gene_set)
